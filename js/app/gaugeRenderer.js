window.FoxGaugeRenderer = (() => {
  ...
  function drawNeedle(root,item,value){const c=item.config||{},shape=shapeName(item),min=Number(c.min??0),max=Number(c.max??8000),start=Number(c.startAngle??225),end=Number(c.endAngle??495),v=c.previewValue??value??min,a=angleFor(v,min,max,start,end),tip=towardCenter(boundary(shape,a,75),.93),back=towardCenter(boundary(shape,a+180,75),.16),g=el("g",{class:"gaugeNeedleGroup"});g.appendChild(el("line",{x1:back.x,y1:back.y,x2:tip.x,y2:tip.y,stroke:c.needleColor||"#e52b2b","stroke-width":14,"stroke-linecap":"round"}));root.appendChild(g);}
