window.FoxGaugeRenderer = (() => {
  ...
  function render(host,item,value){const root=rootSvg(),c=item.config||{},labelColor=c.tickColor||c.textColor||"#ffffff";drawFace(root,item);drawTicks(root,item);drawNeedle(root,item,value);drawHub(root,item);const title=el("text",{x:500,y:350,fill:labelColor,"text-anchor":"middle","font-size":"55","font-weight":"700"});title.textContent=c.title||item.name||"GAUGE";root.appendChild(title);const unit=el("text",{x:500,y:410,fill:labelColor,"text-anchor":"middle","font-size":"34"});unit.textContent=c.unit||"";root.appendChild(unit);const val=el("text",{x:500,y:650,fill:c.textColor||"#ffffff","text-anchor":"middle","font-size":"62","font-weight":"700"});val.textContent=Number.isFinite(Number(value))?Math.round(Number(value)):"0";root.appendChild(val);host.replaceChildren(root);}
