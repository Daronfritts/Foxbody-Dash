window.FoxGaugeRenderer = (() => {
  ...
  function drawTicks(root,item){const c=item.config||{},shape=shapeName(item),start=Number(c.startAngle??225),end=Number(c.endAngle??495),minor=Math.max(1,Number(c.minorTicks??40)),major=Math.max(1,Number(c.majorTicks??8)),every=Math.max(1,Math.round(minor/major)),tickColor=c.tickColor||"#eeeeee",tickScale=clamp(Number(c.tickScale??1),.5,2.5);for(let i=0;i<=minor;i++){const a=start+(end-start)*i/minor,outer=boundary(shape,a,55),isMajor=i%every===0,baseLength=isMajor?.18:.11,length=clamp(baseLength*tickScale,.055,.42),inner=towardCenter(outer,1-length);root.appendChild(el("line",{x1:outer.x,y1:outer.y,x2:inner.x,y2:inner.y,stroke:tickColor,"stroke-width":(isMajor?10:5)*tickScale,"stroke-linecap":"round"}));}for(let i=0;i<=major;i++){const a=start+(end-start)*i/major,p=towardCenter(boundary(shape,a,55),.68),t=el("text",{x:p.x,y:p.y,fill:tickColor,"text-anchor":"middle","dominant-baseline":"middle","font-size":"45","font-weight":"600"});t.textContent=Array.isArray(c.labels)&&c.labels[i]!=null?c.labels[i]:Math.round(Number(c.min??0)+(Number(c.max??100)-Number(c.min??0))*i/major);root.appendChild(t);}}
  ...
  function render(host,item,value){const root=rootSvg(),c=item.config||{},labelColor=c.tickColor||c.textColor||"#ffffff",isLarge=Number(item.w)>=25||Number(item.h)>=35,valueY=isLarge?670:650;drawFace(root,item);drawTicks(root,item);drawNeedle(root,item,value);drawHub(root,item);const title=el("text",{x:500,y:350,fill:labelColor,"text-anchor":"middle","font-size":"55","font-weight":"700"});title.textContent=c.title||item.name||"GAUGE";root.appendChild(title);const unit=el("text",{x:500,y:410,fill:labelColor,"text-anchor":"middle","font-size":"34"});unit.textContent=c.unit||"";root.appendChild(unit);const val=el("text",{x:500,y:valueY,fill:labelColor,"text-anchor":"middle","font-size":"62","font-weight":"700"});val.textContent=Number.isFinite(Number(value))?Math.round(Number(value)):"0";root.appendChild(val);host.replaceChildren(root);}
  function renderPart(host,item,value){const root=rootSvg();if(item.part==="ticks")drawTicks(root,item);else if(item.part==="needle")drawNeedle(root,item,value);else if(item.part==="hub")drawHub(root,item);else if(item.part==="label"){const t=el("text",{x:500,y:500,fill:item.config?.tickColor||item.config?.textColor||"#ffffff","text-anchor":"middle","dominant-baseline":"middle","font-size":"150","font-weight":"700"});t.textContent=item.config?.text||item.name||"LABEL";root.appendChild(t);}else if(item.part==="digital"){const v=value==null||value===""?(item.config?.defaultValue??0):Number(value),dec=item.config?.decimals??0,t=el("text",{x:500,y:500,fill:item.config?.tickColor||item.config?.textColor||"#ffffff","text-anchor":"middle","dominant-baseline":"middle","font-size":"310","font-weight":"700"});t.textContent=Number.isFinite(Number(v))?Number(v).toFixed(dec):"0";root.appendChild(t);}host.replaceChildren(root);}
